...
static void output_blink(void) {
    struct blink_item blink = {.duration_ms = CONFIG_LED_INDICATOR_OUTPUT_BLINK_MS};
...
static int led_battery_listener_cb(const zmk_event_t *eh) {
    ...
    if (battery_level <= CONFIG_LED_INDICATOR_BATTERY_LEVEL_CRITICAL) {
        ...
        struct blink_item blink = {.duration_ms = CONFIG_LED_INDICATOR_BATTERY_BLINK_MS,
                                   .color = LED_RED};
        k_msgq_put(&led_msgq, &blink, K_NO_WAIT);
    }
    return 0;
...
#if IS_ENABLED(CONFIG_LED_INDICATOR_SHOW_LAYER_CHANGE)
#if !IS_ENABLED(CONFIG_ZMK_SPLIT) || IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
static int led_layer_listener_cb(const zmk_event_t *eh) {
    ...
    static const struct blink_item blink = {.duration_ms = CONFIG_LED_INDICATOR_LAYER_BLINK_MS,
                                            .color = LED_CYAN,
                                            .sleep_ms = CONFIG_LED_INDICATOR_LAYER_BLINK_MS};
    static const struct blink_item last_blink = {.duration_ms = CONFIG_LED_INDICATOR_LAYER_BLINK_MS,
                                                 .color = LED_CYAN};
...
#endif
#endif // IS_ENABLED(CONFIG_LED_INDICATOR_SHOW_LAYER_CHANGE)
...
extern void led_init_thread(void *d0, void *d1, void *d2) {
    ...
    if (battery_level >= CONFIG_LED_INDICATOR_BATTERY_LEVEL_HIGH) {
        LOG_INF("Battery level %d, blinking green", battery_level);
        blink.color = LED_GREEN;
    } else if (battery_level >= CONFIG_LED_INDICATOR_BATTERY_LEVEL_LOW) {
    ...
    k_sleep(K_MSEC(CONFIG_LED_INDICATOR_BATTERY_BLINK_MS + CONFIG_LED_INDICATOR_INTERVAL_MS));
#endif // IS_ENABLED(CONFIG_ZMK_BATTERY_REPORTING)
